...
public class SwaggerConfig {
	...
	public Docket api(SwaggerConfigProperties swaggerConfigProperties) {
		return new Docket(DocumentationType.SWAGGER_2).select()
				.apis(RequestHandlerSelectors.basePackage("com.sport")).paths(PathSelectors.any()).build()
				.apiInfo(metaData(swaggerConfigProperties));
	}
	...
	private ApiInfo metaData(SwaggerConfigProperties swaggerConfigProperties) {
		return new ApiInfoBuilder()

				.title(swaggerConfigProperties.getTitle()).description(swaggerConfigProperties.getDescription())
				.version(swaggerConfigProperties.getApiVersion()).license(swaggerConfigProperties.getLicense())
				.licenseUrl(swaggerConfigProperties.getEmail()).contact(new Contact(swaggerConfigProperties.getName(),
						swaggerConfigProperties.getGithub(), swaggerConfigProperties.getEmail()))
				.build();
	}
